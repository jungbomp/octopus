...
export class EbayApiService {
  ...
  async bulkUpdatePriceQuantity(store: StoreType, bulkUpdatePriceQuantityDtos : EbayApiBulkUpdatePriceQuantityDto[]): Promise<number> {
  ...
  async getOrders(store: StoreType, { limit, offset, creationDateFrom: creationFrom, creationDateTo: creationTo, lastModifiedDateFrom: lastModifiedFrom, lastModifiedDateTo: lastModifiedTo }: EbayApiGetOrdersDto): Promise<any> {
    const creationDateFrom: Date = creationFrom && (creationFrom?.lastIndexOf('Z') > -1 ? toDateFromISOString(creationFrom) : toDateFromDateString(creationFrom));
    const creationDateTo: Date = creationTo && (creationTo?.lastIndexOf('Z') > -1 ? toDateFromISOString(creationTo) : toDateFromDateString(creationTo));
    const lastModifiedDateFrom: Date = lastModifiedFrom && (lastModifiedFrom?.lastIndexOf('Z') > -1 ? toDateFromISOString(lastModifiedFrom) : toDateFromDateString(lastModifiedFrom));
    const lastModifiedDateTo: Date = lastModifiedTo && (lastModifiedTo?.lastIndexOf('Z') > -1 ? toDateFromISOString(lastModifiedTo) : toDateFromDateString(lastModifiedTo));
    ...
    if (lastModifiedDateFrom) {
      filter.push(`lastmodifieddate:[${(lastModifiedDateFrom.toISOString())}..${lastModifiedDateTo?.toISOString() ?? ''}]`);
    }
  ...
  async getShippingFulfillments(store: StoreType, orderId: string): Promise<any> {
    return await this.ebayApiCall(`sell/fulfillment/v1/order/${encodeURIComponent(orderId)}/shipping_fulfillment`, `get`, {}, null, store);
  }

  async createShippingFulfillment(store: StoreType, { orderId, lineItems, trackingNumber, shippingCarrierCode, shippedDate: shippedDateStr }: EbayApiCreateShippingFulfillmentDto): Promise<any> {
    const shippedDate: string = shippedDateStr && (shippedDateStr.lastIndexOf('Z') > 0 ? shippedDateStr : toDateFromDateString(shippedDateStr).toISOString());
    const data = {
      lineItems,
      trackingNumber,
      shippingCarrierCode,
    ...
    return await this.ebayApiCall(`sell/fulfillment/v1/order/${encodeURIComponent(orderId)}/shipping_fulfillment`, 'post', {}, JSON.stringify(data), store);
  }
  ...
  async exportAllInventoryItems(store: StoreType): Promise<Readable> {
    ...
    items.forEach(item => {
      tsvRow.push(`${store}\t${item.sku}\t${item.product?.upc}\t${item.product?.title}\t${item.condition}\t${item.availability?.shipToLocationAvailability?.quantity}`);
    });
  ...
  private async generateAuthToken(code: string): Promise<EbayApiAccessTokenType> {
    const headers = {
      ...
      'Authorization': `Basic ${Buffer.from(`${this.ebayApiConfig.appId}:${this.ebayApiConfig.certId}`).toString('base64')}`
    };
    ...
    const token: any = await this.ebayApiCall('identity/v1/oauth2/token', 'post', headers, formBody.join('&'), null, false);
  ...
  private async ebayApiCall(url: string, method: Method, headers: any, data: any, store: StoreType, setAccessToken = true): Promise<any> {
