...
export class ProductOrdersService {
  ...
  async find(orderSeq: number, stdSku?: string): Promise<ProductOrder[]> {
    ...
    return this.productOrdersRepository.find(stdSku ? {
      ...option,
      inventory: { stdSku }
    } : option);
  }
  ...
  async create(createProductOrderDto: CreateProductOrderDto): Promise<ProductOrder> {
    const orderSeq = ((await this.productOrdersRepository.createQueryBuilder('productOrder').orderBy('productOrder.orderSeq', 'DESC').limit(1).getOne()) || { orderSeq: 0 }).orderSeq + 1;
    ...
    const brand = createProductOrderDto.brandCode ?await this.brandsService.findOne(createProductOrderDto.brandCode) : undefined;
    const vendor = createProductOrderDto.vendorCode ? await this.vendorsService.findOne(createProductOrderDto.vendorCode) : undefined;
    const user = createProductOrderDto.employeeId ? await this.usersService.findOne(createProductOrderDto.employeeId) : undefined;
    const productOrder = CreateProductOrderDto.toProductOrderEntity(orderSeq, createProductOrderDto, inventory, product, brand, vendor, user);
  ...
  async createBatch(createProductOrderDtos: CreateProductOrderDto[]): Promise<ProductOrder[]> {
    ...
    const productOrders = createProductOrderDtos.map(dto => {
      const inventory = (stdSku => { const inventory = new Inventory(); inventory.stdSku = stdSku; return inventory; })(dto.stdSku);
      const product = (productCode => { const product = new Product(); product.productCode = productCode; return product; })(dto.productCode);
      const brand = dto.brandCode ? (brandCode => { const brand = new Brand(); brand.brandCode = brandCode; return brand; })(dto.brandCode) : undefined;
      const vendor = dto.vendorCode ? (vendorCode => { const vendor = new Vendor(); vendor.vendorCode = vendorCode; return vendor; })(dto.vendorCode) : undefined;
      const user = dto.employeeId ? (employeeId => { const user = new User(); user.employeeId = employeeId; return user; })(dto.employeeId) : undefined;

      return CreateProductOrderDto.toProductOrderEntity(orderSeq, dto, inventory, product, brand, vendor, user);
    });
  ...
  async exportOrderDetailsToGoogleSheet(createProductOrderDtos: CreateProductOrderDto[]): Promise<any> {
    ...
    let latestFileId = this.orderFormConfig.latestSheetFileId ?? await this.setCurrentGoogleSheetFileId();
    ...
    const productOrders = createProductOrderDtos.map(dto => {
      ...
      return CreateProductOrderDto.toProductOrderEntity(0, dto, inventory, product, brand, vendor, user);
    });
    ...
    const curYearAndMonth = Number(this.dateTimeUtil.getDttmFromDate(curDate).substring(0, 6));
    if (parseInt(latestFileMeta.title.substring(latestFileMeta.title.lastIndexOf('_')+1)) < curYearAndMonth) {
      latestFileId = await this.googleApiService.createSpreadFile(`${latestFileMeta.title.substring(0, latestFileMeta.title.lastIndexOf('_'))}_${curYearAndMonth}`);
      latestFileMeta = await this.googleApiService.getFileMetadata(latestFileId);
      await this.googleApiService.patchFile(latestFileId, latestFileMeta.title, this.orderFormConfig.orderFormFolderId, latestFileMeta.parents[0].id);
      this.orderFormConfig.latestSheetFileId = latestFileId;
    }

    // Create a new sheet into the file
    const day = curDate.getDate();
    const time = this.dateTimeUtil.getTimeFormatFromDate(curDate);
    const title = `${day}${1 === day ? 'st' : 2 === day ? 'nd' : 3 === day ? 'rd' : 'th'} ${time}`;
    const sheetMeta = await this.googleApiService.spreadSheetUpdate(latestFileId, [{ addSheet: { properties: { title }}}]);
    ...
    orderDetails.orderDetails.forEach(orderDetail => {
      orderDetail.orderVariants.forEach((orderVariant, i) => {
        ...
        data.push({
          ...
          values: [ [orderVariant.product.brand.brandName + ((orderDetail.vendor || undefined) !== undefined ? ` (${orderDetail.vendor.vendorName})` : ''), orderVariant.product.productCode, orderVariant.product.productTitle] ]
        });

          data.push({
            range: `${title}!D${curRow}:${lastSizeCol}${curRow+orderVariant.colorVariants.length-1}`,
            values: [ ...orderVariant.colorVariants.map((color, i) => [color, ...orderVariant.orderVariantTable[i].map(v => 0 < v ? ''+v : '')])]
          });
  ...
  private sendLogiwaReceipt(productOrders: ProductOrder[]): void {
    ...
    const receipts = Array.from(orderMap, async ([supplierCode, orders]) => {
      const supplier = orders[0].brand ? (await this.brandsService.findOne(supplierCode)).brandName
      ...
      return new ReceiptDto(
        ...
        orders[0].orderSeq,
        supplier,
        orders.map(order => new ReceiptDetailDto(
          order.inventory.stdSku,
          order.orderQty
        ))
      );
    });

    Promise.all(receipts).then(receipts => {
  ...
  private async getOrderVariantsFromProductOrders(productOrders: ProductOrder[]) {
    const ordersByProductMap = productOrders.reduce((acc, cur) =>
      acc.set(cur.product.productCode, [...(acc.get(cur.product.productCode) || []), cur]), new Map<string, ProductOrder[]>());
    ...
    const orderVariants = productCodes.map(async productCode => {
      ...
      const sizeVariants = productVariants.sort((v1, v2) => v1.sizeOrder - v2.sizeOrder)
      ...
      const variantMatrix = colorVariants.map(color => sizeVariants.map(size => productVariants.find(variant => variant.shortSizeCode === size && variant.productColor === color)?.stdSku));
      const orderVariantTable = colorVariants.map((_, i) => sizeVariants.map((_, j) => (ordersByProduct.find(productOrder => productOrder.inventory.stdSku === variantMatrix[i][j]) || {orderQty: 0}).orderQty));
      const manufacturingColorVariants = colorVariants.map((color, i) => (maunfacturingMaps.find(manufacturingMap => variantMatrix[i].some(stdSku => manufacturingMap.inventory.stdSku === stdSku)) || {manufacturingColor: color}).manufacturingColor || color);
      const sumByColor = orderVariantTable.map(row => row.reduce((acc, cur) => acc + cur, 0));

      return {
        product,
        manufacturingCode: (maunfacturingMaps.find(manufacturingMap => manufacturingMap.product.productCode === product.productCode) || {manufacturingCode: ''}).manufacturingCode || product.productCode,
        manufacturingTitle: (maunfacturingMaps.find(manufacturingMap => manufacturingMap.product.productCode === product.productCode) || {manufacturingTitle: ''}).manufacturingTitle || product.productTitle,
  ...
  private async buildOrderDetailsFromProductOrders(productOrders: ProductOrder[]) {
    const ordersByVendorAndBrand = productOrders.reduce((acc, cur) =>
      acc.set(this.getBrandOrVendorCode(cur), [...(acc.get(this.getBrandOrVendorCode(cur)) || []), cur]), new Map<string, ProductOrder[]>());

    const user = await this.usersService.findOne(productOrders.filter(productOrder => (productOrder.user || null) !== null)
      .map(productOrder => productOrder.user).pop().employeeId);
    ...
    const orderDetails = await Promise.all([...ordersByVendorAndBrand.keys()].map(async code => {
      const vendorProductOrders = ordersByVendorAndBrand.get(code);
      const vendorCode = productOrders.find(productOrder => productOrder.vendor ? productOrder.vendor.vendorCode === code : false)?.vendor.vendorCode;
      const vendor = vendorCode ? await this.vendorsService.findOne(vendorCode) : undefined;
      const brandCode = productOrders.find(productOrder => productOrder.vendor ? false : productOrder.brand.brandCode === code)?.brand.brandCode;
  ...
  private async sendOrderEmails(productOrders: ProductOrder[]) {
    ...
    const curDttm = this.dateTimeUtil.getCurrentDttm();
    const yyyy = curDttm.substring(0, 4);
    const mm = curDttm.substring(4, 6);
    const dd = curDttm.substring(6, 8);
    
    const messages = await Promise.all(orderDetail.orderDetails.map(async vendorOrder => {
      ...
      const employeeName = vendorOrder.user ? `${vendorOrder.user.firstName} ${vendorOrder.user.lastName}` : ordersFrom;
