...
export class LogiwaService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.inventoryItemIdMap = existsSync(this.logiwaApiConfig.inventoryItemMapFilename)
      ? JSON.parse(readFileSync(this.logiwaApiConfig.inventoryItemMapFilename, 'utf-8'))
      : {};
  }
  ...
  async warehouseOrderSearch(searchDto: LogiwaOrderSearchDto): Promise<any> {
    const body = {
      ...
      OrderDate_Start: searchDto.orderDateStart
        ? toLogiwaDateFormat(toDateFromDateString(searchDto.orderDateStart))
        : null,
      OrderDate_End: searchDto.orderDateEnd ? toLogiwaDateFormat(toDateFromDateString(searchDto.orderDateEnd)) : null,
      LastModifiedDate_Start: searchDto.lastModifiedDateStart
        ? toLogiwaDateFormat(toDateFromDateString(searchDto.lastModifiedDateStart))
        : null,
      LastModifiedDate_End: searchDto.lastModifiedDateEnd
        ? toLogiwaDateFormat(toDateFromDateString(searchDto.lastModifiedDateEnd))
        : null,
  ...
  async warehouseOrderShipmentInfoSearch(searchDto: LogiwaShipmentReportSearchDto): Promise<any> {
    const body = {
      WarehouseID: 577,
      PageSize: 200,
      SelectedPageIndex: searchDto.selectedPageIndex,
  ...
  async getAllAvailableToPromiseReportList(): Promise<any> {
    ...
    while (true) {
      ...
      logiwaAvailableToPromiseReportSearchDto.selectedPageIndex =
        logiwaAvailableToPromiseReportSearchDto.selectedPageIndex + 1;
    }
  ...
  async exportLogiwaChannelItemsTSV(channelId?: number): Promise<Readable> {
    ...
    for (let i = 0; i < channelIds.length; i++) {
      ...
      while (true) {
        ...
        logiwaListings.forEach((item: any) =>
          bodies.push([
            item.ChannelID,
